 ...
 * implementation.
 */
public class NaiveBayes_Lifelong_NewRatio extends BaseClassifier {
	...
	public NaiveBayes_Lifelong_NewRatio() {
	}

	public NaiveBayes_Lifelong_NewRatio(FeatureSelection featureSelection2,
	...
	public void train(Documents trainingDocs) {
		...
		for (String featureStr : featureSelection.mpFeatureStrToSelected
				.keySet()) {
			if (knowledge.wordCountInPerClass
					.containsKey(featureStr)) {
				double[] counts = mFeatureStrToCountsMap.get(featureStr);
				double ratio = (counts[0] + param.smoothingPriorForFeatureInNaiveBayes)
						/ (counts[1] + param.smoothingPriorForFeatureInNaiveBayes);

				double[] counts_knowledge = knowledge.wordCountInPerClass
						.get(featureStr);
				double ratio_knowledge = (counts_knowledge[0] + param.smoothingPriorForFeatureInNaiveBayes)
						/ (counts_knowledge[1] + param.smoothingPriorForFeatureInNaiveBayes);

				// double ratio_together = (counts[0] + counts_knowledge[0] +
				// param.smoothingPriorForFeatureInNaiveBayes)
				// / (counts[1] + counts_knowledge[1] +
				// param.smoothingPriorForFeatureInNaiveBayes);
				// System.out.println(featureStr + "\t" + ratio + "\t"
				// + ratio_knowledge + "\t" + ratio_together);

				if ((ratio > param.positiveRatioThreshold && ratio_knowledge < param.negativeRatioThreshold)
						|| (ratio_knowledge > param.positiveRatioThreshold && ratio < param.negativeRatioThreshold)) {
					...
					counts[1] = counts_knowledge[1];
					// double new_count = 1;
					// mTotalCountsPerCategory[0] += new_count - counts[0];
					// counts[0] = new_count;
					// mTotalCountsPerCategory[1] += new_count - counts[1];
					// counts[1] = new_count;
				} else {
					counts[0] += counts_knowledge[0];
					mTotalCountsPerCategory[0] += counts_knowledge[0];
				...
				if (ratio > param.positiveRatioThreshold
						&& ratio_knowledge > param.positiveRatioThreshold) {
					// System.out.println(featureStr + "\t" + "+");
					double new_count = counts[1] * (ratio + ratio_knowledge);
					mTotalCountsPerCategory[0] += new_count - counts[0];
					counts[0] = new_count;
				}
				if (ratio < param.negativeRatioThreshold
						&& ratio_knowledge < param.negativeRatioThreshold) {
					// System.out.println(featureStr + "\t" + "-");
					double new_count = counts[0]
							* (1.0 / ratio + 1.0 / ratio_knowledge);
					mTotalCountsPerCategory[1] += new_count - counts[1];
					counts[1] = new_count;
				}
		...
		for (Feature feature : features) {
			...
			for (int catIndex = 0; catIndex < mCategories.length; ++catIndex) {
				logps[catIndex] += com.aliasi.util.Math
						.log2(probTokenByIndexArray(featureStr, catIndex,
								tokenCounts));
				writer.writeLine(featureStr
						+ " : "
						+ probTokenByIndexArray(featureStr, catIndex,
								tokenCounts));
			}
		}
	 ...
	 * Pr(w|c) = (0.5 + count(w,c)) / (|V| * 0.5 + count(all of w, c)).
	 */
	private double probTokenByIndexArray(String featureStr, int catIndex,
			double[] tokenCounts) {
		double tokenCatCount = tokenCounts[catIndex];
		double totalCatCount = mTotalCountsPerCategory[catIndex];
	...
	public double probToken(String token, String cat) {
		...
		return probTokenByIndexArray(token, catIndex, tokenCounts);
	}
