...
public class MaxEnt extends Classifier implements Serializable {
	...
	public static int getNumParameters(Pipe instancePipe) {
		return (instancePipe.getDataAlphabet().size() + 1)
				* instancePipe.getTargetAlphabet().size();
	}
			...
			scores[li] = parameters[li * numFeatures + defaultFeatureIndex]
					+ MatrixOps
							.rowDotProduct(
									...
									fv,
									defaultFeatureIndex,
									(perClassFeatureSelection == null ? featureSelection
											: perClassFeatureSelection[li]));
		}
	}

	public void getClassificationScores(Instance instance, double[] scores) {
		double score = getUnnormalizedClassificationScores(instance);
		scores[0] = 1.0 / (1 + Math.exp(score));
		scores[1] = 1 - scores[0];
	}

	public double getUnnormalizedClassificationScores(Instance instance) {
		// arrayOutOfBounds if pipe has grown since training
		// int numFeatures = getAlphabet().size() + 1;
		int numFeatures = this.defaultFeatureIndex + 1;
		...
		return parameters[defaultFeatureIndex]
				+ MatrixOps.rowDotProduct(parameters, numFeatures, 0, fv,
						defaultFeatureIndex, featureSelection);
	}
	...
	public void getClassificationScoresWithTemperature(Instance instance,
			double temperature, double[] scores) {
		getUnnormalizedClassificationScores(instance, scores);

		// scores should be divided by temperature, scores are sum of weighted
		// features
	...
	public Classification classify(Instance instance) {
		...
		return new Classification(instance, this, new LabelVector(
				getLabelAlphabet(), scores));
	}
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		if (version != CURRENT_SERIAL_VERSION)
			throw new ClassNotFoundException(
					"Mismatched MaxEnt versions: wanted "
		...
		if (nfs >= 0) {
			...
			for (int i = 0; i < nfs; i++) {
				opt = in.readInt();
				if (opt == 1)
					perClassFeatureSelection[i] = (FeatureSelection) in
							.readObject();
			}
		}
	}
