...
public class MaxEntOptimizableByLabelLikelihood implements
	...
	double[] parameters, cachedGradient;
	...
	public double getValue() {
		if (cachedValueStale) {
			...
			while (iter.hasNext()) {
				...
				int li = labeling.getBestIndex();
				int otherli = 1 - li;
				value = scores[li] - scores[otherli];
				cachedValue += value;
	...
	public void getValueGradient(double[] buffer) {
		...
		if (cachedGradientStale) {
			...
			while (iter.hasNext()) {
				...
				this.theClassifier.getClassificationScores(instance, scores);
				double score = this.theClassifier
						.getUnnormalizedClassificationScores(instance);

				FeatureVector fv = (FeatureVector) instance.getData();
				if (li == 0) {
					for (int vli = 0; vli < fv.numLocations(); vli++) {
						double x = fv.valueAtLocation(vli);
						double gradient = score * x / (Math.exp(score) + 1)
								- score * x * (Math.exp(score) - 1)
								/ (Math.exp(score) + 1) / (Math.exp(score) + 1);
						cachedGradient[fv.indexAtLocation(vli)] += 100 * gradient;
					}
				} else {
					for (int vli = 0; vli < fv.numLocations(); vli++) {
						double x = fv.valueAtLocation(vli);
						double gradient = -score * x / (Math.exp(score) + 1)
								- score * x * (1 - Math.exp(score))
								/ (Math.exp(score) + 1) / (Math.exp(score) + 1);
						cachedGradient[fv.indexAtLocation(vli)] += 100 * gradient;
					}
				}
	 ...
	 * @return This trainer
	 */
	public MaxEntOptimizableByLabelLikelihood setGaussianPriorVariance(
	...
	public MaxEntOptimizableByLabelLikelihood setHyperbolicPriorSlope(
	...
	public MaxEntOptimizableByLabelLikelihood setHyperbolicPriorSharpness(
